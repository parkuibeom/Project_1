    ...
    <body>
            ...
            <div class="board_view_wrap">
				...
				<c:choose>
					<c:when test = "${board.getUserId() == user_id }">
                     	<a href="boardEditView.do?boardId=${board.getBoardId() }" class="on">수정</a>
					</c:when>
				</c:choose>
                </div>
                
           
        <form action="comment.do" method="post" name="commentform" id="commentform">
		...
		<div class="comments-container">
			...
			<c:forEach items="${list}" var="dto">
				<div class="comment ${dto.getcStep() == 0 ? 'comment' : 'special-comment'}">
					<c:choose>
						...
						<c:when test="${user_id != null}">
							...
							<c:if test="${dto.getUserId() == user_id}">
								<div class="delete-button">
									<a href="commentDelete.do?group=${dto.getGroup() }&cId=${dto.getcId() }&cStep=${dto.getcStep() }&boardId=${board.getBoardId() }">삭제하기</a>
								</div>
							</c:if>
						</c:when>
						<c:otherwise>
